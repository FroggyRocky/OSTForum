...
class Auth {
    ...
    async compileConfigs(req, res) {
        try {
            ...
            const serviceKeys = await db.ServiceKeys.findAll()
            const affiliateKeys = await db.AffiliateKeys.findAll()
            const vacancyKeys = await db.VacancyKeys.findAll()
            const caseKeys = await db.CaseKeys.findAll()
            const networkKeys = await db.NetworkKeys.findAll()
            const keys = {
                network:networkKeys,
                case:caseKeys,
                vacancy:vacancyKeys,
